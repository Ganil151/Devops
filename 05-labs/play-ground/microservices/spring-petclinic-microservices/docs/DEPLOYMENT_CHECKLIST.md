...
│   ├── inventory/                  # Target environment definitions
│   │   ├── dynamic_hosts           # Generated by Terraform outputs
│   │   └── group_vars/             # Environment-specific variables
│   └── playbooks/                  # System hardening & Tooling setup
│       ├── install-tools.yml       # Java 21, Docker, AWS CLI, kubectl
│       └── security-patch.yml      # OS-level hardening (AL2023)
├── cicd-implementation.md          # Architectural blueprints for Jenkins Pipelines
├── docs/                           # SRE & Operational Documentation
│   ├── DEPLOYMENT_CHECKLIST.md     # (You are here) Step-by-step prod guide
│   ├── JAVA21_MIGRATION.md         # Guide for upgrading microservices
│   └── RUNBOOK_INCIDENT.md         # On-call guide for microservice failures
├── helm/                           # Kubernetes Orchestration (Helm v3)
│   └── petclinic-chart/            # Umbrella chart for all microservices
│       ├── charts/                 # Sub-charts for each MS (API, Vets, etc.)
│       ├── templates/              # Shared K8s manifests (Ingress, WAF)
│       └── values-dev.yaml         # Dev-specific overrides (replicas, limits)
├── scripts/                        # Automation & Developer Experience (DX)
│   ├── aws-auth.sh                 # ECR Login & Kubeconfig synchronization
│   ├── apply.sh                    # Orchestrates: TF -> Ansible -> Helm
│   └── cleanup-resources.sh        # FinOps tool for destroying dangling resources
├── terraform/                      # Infrastructure as Code (AWS Provider)
│   ├── environments/               # Root Workspaces
│   │   ├── dev/                    # Development (Small instances, single AZ)
│   │   ├── staging/                # Staging (Mirror of Prod)
│   │   └── prod/                   # Production (Multi-AZ, High HA)
│   ├── modules/                    # Reusable "Service-Oriented" Modules
│   │   ├── networking/             # VPC, NAT-GW, Transit-GW logic
│   │   ├── eks/                    # Managed K8s & Fargate Profiles
│   │   ├── rds/                    # Aurora/MySQL Multi-AZ clusters
│   │   ├── alb/                    # Application Load Balancer & TLS setup
│   │   ├── ecr/                    # Private Container Registries
│   │   └── monitoring/             # CloudWatch Logs & SNS Alarms
│   └── shared/                     # Global state, providers, & backend lock
└── testing/                        # Automated Quality Gates
    ├── infra/                      # Pytest-Testinfra (Server state auditing)
    ├── security/                   # OPA/Checkov (IaC Scan) & Trivy (Image Scan)
    ├── smoke/                      # Pytest-Requests (End-to-End API verify)
    └── unit/                       # Maven/JUnit results for Java microservices
```
