...
- [ ] **Multi-Environment Pipeline Auth**: Jenkins Credentials configured for both `dev` and `prod`
  - **Details**: Add `aws-jenkins-credentials-dev` and `aws-jenkins-credentials-prod` to the Jenkins Credentials Store.
- [ ] **Webhook & Branch Mapping**: GitHub triggers correct pipeline stages
  - **Details**: Ensure `dev` branch triggers the Dev pipeline and `main` triggers the Prod pipeline with manual approval gates.
- [ ] **Notification Loop**: Slack/Teams notification sent on Build Failure
  - **Details**: Implement environment-specific notification channels for immediate developer alerts.
- [ ] **Environment-Specific Rollback**: Automatic kubectl rollout undo per namespace
  - **Details**: Verify that failure in `prod` namespace triggers an immediate rollback without affecting `dev`.

### Enterprise Infrastructure Validation
- [ ] **State Isolation**: Verify separate S3 buckets for `dev` and `prod` states
  - **Details**: Check `backend.conf` in both `environments/dev` and `environments/prod` directories.
- [ ] **FinOps Guardrails**: Verify Spot Instance configuration in EKS (Dev only)
  - **Details**: Production should typically use Reserved or On-Demand instances, while `dev` uses `SPOT` for cost savings.
- [ ] **IaC Consistency**: Run `terraform validate` across all environment directories
  - **Details**: Ensure the enterprise tree structure is strictly followed and all modules are correctly versioned.