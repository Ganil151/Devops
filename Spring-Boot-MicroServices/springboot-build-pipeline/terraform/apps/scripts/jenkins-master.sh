...
sudo apt update -y
sudo apt install -y openjdk-8-jdk openjdk-11-jdk openjdk-21-jdk git wget gnupg2 ca-certificates apt-transport-https software-properties-common
...
# Maven is typically installed to /usr/share/maven on Ubuntu
...
# Add Jenkins repository key and source list for Ubuntu/Debian
