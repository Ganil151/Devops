...
Credentials
> **⚠️ Missing Image**: *alt text* ('<Image\Screenshot (195').png>)
...
### ---- Jenkins Pipeline Script before MySql implementation ----
```groovy
pipeline {
    agent { label params.NODE_LABEL }
                ...
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentials']]) {
                    ...
                    if [ "$INSTANCE_ID" == "None" ] || [ -z "$INSTANCE_ID" ]; then
                      ...
                      INSTANCE_ID=$(aws ec2 run-instances \
                        ...
                        --instance-type t3.small \
                        --key-name sis_keys \
                        --security-group-ids sg-071975762f8750174 \
                        --subnet-id subnet-0141c3618fdf7cc51 \
                        --associate-public-ip-address \
                        --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${EC2_INSTANCE_NAME}}]" \
                        --query "Instances[0].InstanceId" \
                        --output text)
            ...
            steps {
                sshagent(['sis_keys']) {
                    withCredentials([usernamePassword(credentialsId: 'dockerhub-credentials', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                        ...
                        sudo docker compose version || { echo 'Docker Compose not working'; exit 1; }
