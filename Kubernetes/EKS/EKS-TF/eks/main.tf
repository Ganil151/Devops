...
module "eks" {
  source = "/home/ganil/Documents/Devops/Kubernetes/EKS/EKS-TF/modules"

  env                   = var.env
  cluster_name          = "${local.env}-${local.org}-${var.cluster-name}"
  cluster-name          = "${local.env}-${local.org}-${var.cluster-name}" # Keep for backward compatibility
  ...
  is_eks_cluster_enabled        = var.is-eks-cluster-enabled
