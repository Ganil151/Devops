#!/usr/bin/env bash
#
# Fedora 43 Network Sentinel (Audit, Secure, & Repair) - FINAL EDITION
# Architected for: Fedora 43 (DNF5, Systemd-Resolved, Firewalld, SELinux)
# Safety: Dry-run default, backups before changes, explicit --apply required
# Author: Ganil Batist Yan (DevOps Engineer)
#
...
safe_resolved_conf_edit() {
    ...
    if command -v systemd-conf &>/dev/null && [[ "$DRY_RUN" == false ]]; then
        ...
        systemd-conf set "$file" "$key" "$value" --backup-dir="$BACKUP_DIR" 2>/dev/null || {
            # Fallback to sed if systemd-conf fails
...
get_active_zone() {
    ...
    local output zones
    output=$(firewall-cmd --get-active-zones 2>/dev/null || echo "")
    if [[ -z "$output" ]]; then
        echo ""
        return
    fi
    # Extract first zone name (skip headers like "interfaces:")
    zones=$(echo "$output" | awk '/^[a-zA-Z]/{print $1; exit}')
    echo "$zones"
}
...
audit_dns() {
    ...
    # Check resolved mode
    local resolved_mode
    resolved_mode=$(resolvectl status 2>/dev/null | grep "resolv.conf mode" | awk '{print $4}' || echo "unknown")
    ...
    # DNS protocol status
    resolvectl status 2>/dev/null | grep -E 'Protocols|DNS Servers' | head -10 || true
...
verify_fixes() {
    ...
    # Git connectivity (CORRECTED: use valid repo URL)
    if command -v git &>/dev/null; then
        if timeout "$((TIMEOUT*2))" git ls-remote --exit-code https://github.com/octocat/Hello-World.git >/dev/null 2>&1; then
            log "$GREEN" "✅ Git HTTPS connectivity: SUCCESS"
        else
            log "$YELLOW" "⚠️  Git connectivity test failed - check:"
            log "$YELLOW" "   - Git config: git config --global --list"
            log "$YELLOW" "   - SSL certs: sudo update-ca-trust extract"
            log "$YELLOW" "   - Proxy: git config --global http.proxy"
        fi
    fi
...
1. Restore config backups:
   sudo tar -xzf ${BACKUP_DIR}/*.tar.gz -C / 2>/dev/null || echo "No tarball found"
...
3. Restore resolved.conf:
   sudo cp ${BACKUP_DIR}/resolved.conf.* /etc/systemd/resolved.conf 2>/dev/null || echo "No backup found"
