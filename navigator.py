...
class ModuleScanner:
    ...
    def _parse_readme(self, path: Path) -> Optional[Dict]:
        ...
        try:
            ...
            tag_match = re.search(r'(?:^|\s)#?tags:\s*(.*)', content, re.IGNORECASE | re.MULTILINE)
            if tag_match:
...
class Navigator:
    ...
    def setup_bindings(self):
        ...
        def _(event):
            self.selected_index = min(len(self.filtered_modules) - 1, self.selected_index + 1)

        for key, phase in [('f1', 'Phase 1'), ('f2', 'Phase 2'), ('f3', 'Phase 3'), ('f4', 'All')]:
    ...
    def on_enter(self, buffer):
        ...
        def run_action():
            console.clear()
            console.print(Panel(f"[bold cyan]{selected['title']}[/bold cyan]\n[dim]{selected['path']}[/dim]", title="Selected Module"))
            console.print("\n[bold]Actions:[/bold]")
    ...
    def get_formatted_list(self):
        ...
        for i, m in enumerate(self.filtered_modules[start:end]):
            ...
            p_color = "white"
            if m['phase'] == "Phase 1": p_color = "blue"
            elif m['phase'] == "Phase 2": p_color = "green"
            elif m['phase'] == "Phase 3": p_color = "yellow"
    ...
    def run(self):
        root = HSplit([
            Window(FormattedTextControl(lambda: f" DevOps Navigator | Filter: {self.active_phase} (F1-F3, F4=All) | Found: {len(self.filtered_modules)} "), height=1, style="class:header"),
            self.search_field,
            Window(FormattedTextControl(self.get_formatted_list))
        ])
        
        style = Style.from_dict({"header": "bg:#333333 #ffffff bold", "search-field": "bg:#222222 #ffffff"})
        Application(layout=Layout(root), key_bindings=self.bindings, style=style, full_screen=True).run()
