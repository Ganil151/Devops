...
**Situation**: You are building a mobile game with millions of players worldwide. You need to show top scores in real-time.
**Challenge**: Traditional databases struggle with millions of concurrent writes and sorting data globally.
**Solution**:
1.  **Data Modeling**: Use `GameId` as Partition Key and `Score` as Sort Key to efficiently query top scores.
2.  **Global Tables**: Enable Global Tables to replicate data across US, EU, and Asia regions, allowing players to read/write to their local endpoint with low latency.
3.  **Performance**: Use On-Demand capacity to handle sudden spikes when usage goes viral.

### Scenario 2: E-Commerce Shopping Cart (Black Friday)
**Situation**: An online retailer expects 50x traffic on Black Friday. The "Add to Cart" functionality must NEVER fail.
**Challenge**: Providing enough provisioned capacity for the peak is expensive and hard to predict perfectly.
**Solution**:
1.  **On-Demand Mode**: Switch the DynamoDB table to On-Demand capacity mode. It instantly accommodates the spike without pre-warming.
2.  **Availability**: DynamoDB's built-in Multi-AZ replication ensures the cart data is safe even if a data center fails.
3.  **TTL**: Set a Time-To-Live (TTL) on cart items to auto-delete abandoned carts after 7 days, keeping storage costs low.

### Scenario 3: Ride-Sharing Geo-Data
**Situation**: A ride-sharing app needs to store trip history for millions of rides.
**Solution**:
1.  **Partition Key**: Use `RideId` for unique identification.
2.  **GSI**: Create a Global Secondary Index on `DriverId` (Partition Key) and `Date` (Sort Key) to allow drivers to view their trip history sorted by time.
3.  **Streams**: Enable DynamoDB Streams to trigger a Lambda function that calculates daily earnings whenever a new ride is completed.

---

# 🎓 Interview Questions

**Q1: What is the difference between a Scan and a Query in DynamoDB?**
**Answer**:
*   **Query**: Finds items based on the Primary Key (Partition Key matches value, Sort Key optional condition). It is efficient and predictable.
*   **Scan**: Reads every single item in the table to find matches. It is slow, expensive, and consumes a lot of capacity. Avoid Scans in production whenever possible.

**Q2: Can you add a Local Secondary Index (LSI) to an existing table?**
**Answer**: No. LSIs must be defined at the time of table creation. If you need an index later, you must create a Global Secondary Index (GSI), which can be added or removed at any time.

**Q3: Explain the difference between Eventually Consistent and Strongly Consistent reads.**
**Answer**:
*   **Eventually Consistent** (Default): Reads might reflect data from a stale replica (usually <1 sec old). Consumes 0.5 RCU per 4KB.
*   **Strongly Consistent**: Reads always reflect the latest successful write. Consumes 1.0 RCU per 4KB. Use this only when absolutely necessary (e.g., checking inventory).
