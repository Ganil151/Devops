...
## 📚 Overview
In DevOps, you rarely write code from scratch. You spend 80% of your time reading logs, debugging errors, and searching through configuration files. `grep` (Global Regular Expression Print) is the ultimate tool for this. It allows you to find specific text patterns across thousands of files instantly.
...
graph LR
    ...
    Engine -- No Match --> Null["🗑️ /dev/null"]
...
- `fgrep`: Fixed string (Same as `grep -F`). Treats characters literally. Faster for simple strings.
</details>
...
2. **What does `grep -v` do?**
   - [ ] a) Verbose mode
   - [x] b) Invert match (Show lines NOT matching)
   - [ ] c) Verify match
   - [ ] d) View file
...
4. **Why use `fgrep` (or `grep -F`)?**
   - [ ] a) It uses colors
   - [x] b) It treats patterns as fixed strings
