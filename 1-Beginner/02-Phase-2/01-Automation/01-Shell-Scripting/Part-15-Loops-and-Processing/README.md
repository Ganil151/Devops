...
2. ✅ **Implement While-Read loops for high-performance line processing.**
   ...
   - **Use Case**: Parsing a 404 access log to find errors.
...
- **Shell Globbing (The Gold Standard)**: `for file in *.conf; do ... done`. Globbing is performed by the shell's internal engine *before* the loop starts. It is the only safe way to iterate over files because it treats the entire filename as a single token, preserving spaces and special characters perfectly.
- **The `$(ls)` Trap (Production Poison)**: **NEVER** use `for i in $(ls)`. This creates a "subshell ghost" where the output of `ls` is treated as a single string and then re-split by the shell using `$IFS`. If a file is named `Backup Site.conf`, the loop will treat it as two separate files: `Backup` and `Site.conf`.
...
The `select` loop is a specialized Bash construct for building **CLI Menus** with minimal code.

- **Mechanism**: It generates a numbered list from a collection and prompts the user for a choice.
- **Variable**: The user's selection is stored in the variable provided, and the raw input is stored in `$REPLY`.
...
### Pattern C: Parallelism with `xargs` (The Pro Choice)

The `for ... &` pattern is great for small tasks, but it doesn't limit the number of processes. To prevent crashing your CPU, use `xargs` with the `-P` (Parallel) flag.

```bash
# Process 5 logs concurrently at any given time
find /var/log/app -name "*.log" | xargs -I {} -P 5 gzip {}
```
