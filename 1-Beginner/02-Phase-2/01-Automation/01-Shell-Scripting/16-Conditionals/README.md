...
One of the most common points of failure in Shell scripting is using the wrong comparison tool. "It works on my machine" often turns into "It crashed in production" because of subtle differences between brackets.

### 1. The Comparison Matrix

| Feature | `[ ... ]` (Single) | `[[ ... ]]` (Double) | `(( ... ))` (Parens) |
| :--- | :--- | :--- | :--- |
| **Type** | External Command (POSIX) | Bash Keyword | Arithmetic Context |
| **Primary Use** | Legacy / Portability (`sh`) | Modern Bash Strings/Files | Integers / Math |
| **Empty Variables** | Crashes (Syntax Error) | Handles Safely | Treated as `0` |
| **Word Splitting** | Yes (Must Quote Vars) | No (Safe without Quotes) | No |
| **Logical Operators** | `-a`, `-o` (Deprecated) | `&&`, `\|\|` | `&&`, `\|\|` |
| **Pattern Matching** | No | Yes (Globs & Regex) | No |
| **Operators** | `-eq`, `-gt`, `=` | `==`, `!=`, `=~` | `==`, `>`, `<=`, `+` |

### 2. The Modern Standard: `[[ ... ]]` (Strings & Files)

Always use double brackets for string comparisons and file tests. They are a "Bashism" (specific to Bash, Zsh, etc.) that provides built-in protection against common pitfalls.

-   **Safety**: Variables don't vanish when empty. `[[ $empty == "val" ]]` is valid; `[ $empty == "val" ]` is an error.
-   **Glob Matching**: Match patterns without regex.
    ```bash
    [[ $filename == *.tar.gz ]] && echo "This is a tarball"
    ```
-   **Regex Matching**: Use `=~` for advanced validation.
    ```bash
    [[ $email =~ ^[A-Za-z0-9._%+-]+@example\.com$ ]]
    ```

### 3. The Math Specialist: `(( ... ))` (Integers)

When working purely with numbers, use double parentheses. This mode behaves like C-language arithmetic, allowing clean, readable code.

-   **Readable Operators**: Use `>`, `<`, `>=`, `<=` instead of `-gt`, `-lt`, `-ge`, `-le`.
-   **Logic + Math**: Supports inline calculation.
    ```bash
    # Increment 'retries' and check limit in one step
    while (( retries++ < 5 )); do
        try_connect && break
    done
    ```
-   **Ternary Logic**: Supports `condition ? true_val : false_val`.
    ```bash
    (( result = score > 50 ? 1 : 0 ))
    ```
...
-   **The Quote Trap (`[ ]`)**:
    -   *Bad*: `[ $name == "Steve Jobs" ]` fails because bash sees `[ Steve Jobs == Steve Jobs ]` (Too many arguments).
    -   *Good*: `[[ $name == "Steve Jobs" ]]` handles spaces natively.
-   **Assignment vs Equality**:
    -   *Bad*: `if [ $a = $b ]` (In `[ ]`, `=` checks string equality, but looks like assignment to programmers).
    -   *Good*: `if [[ $a == $b ]]` (Explicit equality, less ambiguous).
-   **The Float Fallacy**:
    -   Bash **cannot** handle floating point numbers (e.g., `0.5`). `(( 1.5 > 1 ))` is a syntax error.
    -   *Fix*: Use `bc` or `awk` for floats.
-   **Octal confusion**:
    -   `(( 010 == 8 ))` is True! Leading zeros trigger Octal mode. Be careful with padded dates like "08" or "09" (invalid octal).
    -   *Fix*: Force base-10 with `10#`: `(( 10#$var ))`.
...
7. **Q: Can you combine logical AND/OR inside a `case` statement?**
   - *A: Not directly in the pattern matcher like `if [[ a && b ]]`. However, you can use the pipe `|` for OR logic (e.g., `start|up)`). For logical AND, you typically nesting nested `case` or `if` inside the block.*
...
Proceed to: **[Loops](../17-For-Loops/README.md)** →
