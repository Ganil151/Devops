...
Early Unix shells lacked built-in math capabilities, forcing engineers to spawn external processes like `expr` for even simple addition. This was slow and cumbersome. Modern Bash introduced the `$(( ... ))` arithmetic expansion, which is executed natively by the shell. For complex decimal math, we still integrate the `bc` (Basic Calculator) utility, bridgeing the gap between the shell's integer-only logic and the precise needs of metric monitoring.
...
### Pattern C: Precision Scaling

When using `bc`, you can set the `scale` variable to control the number of decimal places.

```bash
# Calculate average load to 2 decimal places
AVG=$(echo "scale=2; 10.55 / 3" | bc)
echo "Average: $AVG" # 3.51
```
...
Proceed to: **[Vim Crash Course](../11-Vim-Crash-Course/README.md)** →
