...
![OSI 7 Layers](./Images/OSI-7-layers.jpg.webp)
...
As data moves down the OSI layers, each layer adds its own header (and sometimes a trailer). This process is called **Encapsulation**.
...
- OSI layers 1, 2 are combined into one Network Access Layer in TCP/IP – however this layer does not take responsibility for sequencing and acknowledgement functions, leaving these to the upper transport layer (TCP).
...
### ⏭️ Next Step
Take a deep dive into each layer starting with the [Physical Layer](./OSI%20Model/1.%20Physical/README.md).