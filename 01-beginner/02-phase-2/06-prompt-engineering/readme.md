...
- **Output Schemas**: Forcing the AI to speak JSON, not English, so it can be parsed by CI/CD pipelines.
...
- **Data Privacy**: The absolute prohibition of sending PII/secrets to public model endpoints.
- **Hallucination Management**: Implementing "Grounding" techniques to prevent the AI from inventing non-existent libraries or commands.
...
| **[⌨️ SOH](./slash-commands.md)** | **Slash Commands** | - **Slash Commands**: Rapid "SRE-style" codes for de-robotizing and formatting. (See [Phase 2: Prompt Engineering](./01-beginner/02-phase-2/06-prompt-engineering/slash-commands.md)) |
